...
export const __addComment = createAsyncThunk(
    ...
    async (payload, thunkAPI) => {
        try {
            //id Max값 여기서 구현해봄
            const getComments = await axios.get(`${process.env.REACT_APP_API_URL_COMMENTS}`);
            const commentsIdArr = getComments.data.map((e) => { return e.id })
            await axios.post(`${process.env.REACT_APP_API_URL_COMMENTS}`, { ...payload, id: (Math.max(commentsIdArr) + 1), isMod: false });
            const data = await axios.get(`${process.env.REACT_APP_API_URL_COMMENTS}`);
...
export const __deleteComment = createAsyncThunk(
    ...
    async (payload, thunkAPI) => {
        try {
            await axios.delete(`${process.env.REACT_APP_API_URL_COMMENTS}/${payload.id}`, { data: { contentId: Number(payload.contentId) } });
            const data = await axios.get(`${process.env.REACT_APP_API_URL_COMMENTS}`);
            const filterData = data.data.filter((val) => { return val.contentId === payload.contentId });
            return thunkAPI.fulfillWithValue(filterData);
        } catch (error) {
...
export const __updateComment = createAsyncThunk(
    ...
    async (payload, thunkAPI) => {
        try {
            await axios.patch(`${process.env.REACT_APP_API_URL_COMMENTS}/${payload.id}`, payload);
            const data = await axios.get(`${process.env.REACT_APP_API_URL_COMMENTS}`);
            const filterData = data.data.filter((val) => { return val.contentId === payload.contentId });
            return thunkAPI.fulfillWithValue(filterData);
        } catch (error) {
